...
// Returns text using the toUpperCase method. Makes everything uppercase
const upperCaseText = (text) => {
  return `${text.toUpperCase()}!`;
};

console.log(upperCaseText("dette er kult"));
...
// If time is undefined (not defined), return error message
// If time is less than 0 or more than 23, return "Ugyldig tid"
// If time is less or equal to 5, return `God natt ${name}`
// If time is less or equal to 11 (Not counting 5 and lower), return `God morgen ${name}`
// If time is less or equal to 17 (Not counting 11 and lower), return `God dag ${name}`
// else (more than 17), return `God kveld ${name}`

function greeting(name, time) {
  if (time === undefined) {
    return "Error: Time verdi ikkje funnet";
...
const colors = ["Rød", "Grønn", "Blå", "Gull"];
const numbers = [1, 2, 3, 4, 5, 6];

// Removes the first index of an array
...
console.log(removeFirstAndLast(numbers));
...
// Text variable has the value changed to itself using the replace method. Replaces one word with another.
// Returns the modified text while using trim. Removes whitespace from start and end on the string.
function funChanger(text) {
  text = text.replace("vanskelig", "gøy");
  return text.trim();
}

console.log(funChanger("   dette er vanskelig!   "));
...
// Extra
// Filters the array to only include index that has "e" in it.
const newArray = items.filter((items) => items.includes("e"));
